#include "tf/tf.h"
#include <mobile_robot_simulator/obstacle_simulation.hh>
...
void ObstacleSimulation::processFeedback(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback){
  if (transform_flag_){
    ...
    mobile_robot_simulator::Obstacle msg;
    msg.request.obstacle_id = obstacle_id_;
    msg.request.type = mobile_robot_simulator::Obstacle::Request::TRANSFORM;
...
void ObstacleSimulation::menuFeedback( const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback ){
  ...
  if (state == interactive_markers::MenuHandler::CHECKED){
    msg.request.type = mobile_robot_simulator::Obstacle::Request::DELETE;
    if (client_.call(msg)){
    ...
    msg.request.type = mobile_robot_simulator::Obstacle::Request::NEW;
...
void ObstacleSimulation::run(){
  ...
  client_ = nh_.serviceClient<mobile_robot_simulator::Obstacle>("/obstacle_handle");
  server_->applyChanges();
  ros::spin();
}
